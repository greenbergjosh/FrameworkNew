...
namespace Framework.Core.GenericEntity
{
    ...
    public class GenericEntityMultiIndexCache<TObject>
    {
        ...
        {
            bool usePKey = pKey != null && pKey != string.Empty;
            bool useSKey = index != null && index != string.Empty && sKey != null && sKey != string.Empty;

            if (!usePKey && !useSKey) throw new InvalidOperationException("No key supplied");
