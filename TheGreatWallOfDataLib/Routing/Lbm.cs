...
namespace TheGreatWallOfDataLib.Routing
{
    public static class Lbm
    {
        ...
        public static async Task<IGenericEntity> Run(string idOrName, string payload, string identity, HttpContext ctx)
        {
            ...
            try
            {
                return (IGenericEntity)(await _fw.RoslynWrapper.Evaluate(id.Value, code.GetS("Config"), new { _httpContext = ctx, _payload = payload, _fw, _lbmConfig = lbm }, new StateWrapper()));
            }
            catch (Exception e)
            {
