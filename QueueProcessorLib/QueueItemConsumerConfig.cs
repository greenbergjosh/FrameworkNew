...
namespace QueueProcessorLib
{
    internal sealed class QueueItemConsumerConfig
    {
        ...
        {
            var itemsInFlightMaxCount = int.Parse((await fw.StartupConfiguration.GetS("QueueProcessor.QueueItemConsumer.ItemsInFlightMaxCount")).IfNullOrWhitespace("50"));
            var sleepInterval = int.Parse((await fw.StartupConfiguration.GetS("QueueProcessor.QueueItemConsumer.SleepInterval")).IfNullOrWhitespace("1000"));
            var drainInterval = int.Parse((await fw.StartupConfiguration.GetS("QueueProcessor.QueueItemConsumer.DrainInterval")).IfNullOrWhitespace("60000"));
            var drainSleepInterval = int.Parse((await fw.StartupConfiguration.GetS("QueueProcessor.QueueItemConsumer.DrainSleepInterval")).IfNullOrWhitespace("1000"));
