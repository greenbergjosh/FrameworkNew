...
namespace QueueProcessorLib
{
    internal sealed class QueueItemProducerConfig
    {
        ...
        {
            var sleepInterval = int.Parse((await fw.StartupConfiguration.EvalS("QueueProcessor.QueueItemProducer.SleepInterval")).IfNullOrWhitespace("1000"));
            var batchSize = int.Parse((await fw.StartupConfiguration.EvalS("QueueProcessor.QueueItemProducer.BatchSize")).IfNullOrWhitespace("20"));
            var lowWatermark = int.Parse((await fw.StartupConfiguration.EvalS("QueueProcessor.QueueItemProducer.LowWatermark")).IfNullOrWhitespace("10"));

            var highWatermark = int.Parse((await fw.StartupConfiguration.EvalS("QueueProcessor.QueueItemProducer.HighWatermark")).IfNullOrWhitespace("20"));
