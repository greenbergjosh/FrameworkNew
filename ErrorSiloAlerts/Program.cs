    ...
    class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            try
            {
                ...
                var lastSeqIds = (await fw.Data.ExecuteMethod("History", "LastSeqIds", "", "")).GetL("").ToDictionary(d => d.GetS("ConfigName"), d => d.GetS("LastSeqId").ParseLong().Value);
                ...
                using (var smtp = new SmtpClient(smtpRelay, smtpPort))
                {
                    ...
                    foreach (var (configName, lastSeqId, msg) in emails)
                    {
                        ...
                        {
                            smtp.Send(msg);
                            var res = await fw.Data.ExecuteMethod("History", "InsertAlertsSent", Jw.Json(new { configName, lastSeqId }), "");
        ...
        private static async Task<IEnumerable<(string configName, long lastSeqId, MailMessage msg)>> GetEmailAlertDescriptorEvents(Dictionary<string, long> lastSeqIds, DateTime minTimestamp)
        {
            ...
            var rawEmailAlerts = await fw.Data.ExecuteMethod("ErrorWarehouse", "EmailAlerts", Jw.Json(new { lastSeqId = minLastSeqId, minTimestamp }), "");
