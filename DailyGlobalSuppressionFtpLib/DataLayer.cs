...
namespace DataLayerExtensions
{
    public static class DataTableExtensions
    {
        ...
        {
            DailyGlobalSuppressionFtpLib.DataLayer.AddDataRow(dt, data);
        }
    }
}
...
namespace DailyGlobalSuppressionFtpLib
{
    ...
    { 
        ...
        public static async Task<bool> AlreadyDownloaded(string connectionString, string fileName, byte suppressionFileTypeId)
        {
            ...
            SqlDataReader sdr;
            SqlCommand cmd = cn.CreateCommand();
            cmd.CommandText = "DailyGlobalSuppressionFtp.AlreadyDownloaded";
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Add(new SqlParameter("FileName", fileName));
            cmd.Parameters.Add(new SqlParameter("SuppressionFileTypeId", suppressionFileTypeId));
        ...
        public static async Task InsertProcessedFtpFile(string connectionString, byte suppressionFileTypeId, string fileName, DateTime fileDate, DateTime startDate, DateTime endDate, long recordsInFile, long recordsAddedFromFile)
        {
            SqlConnection cn = new SqlConnection(connectionString);
            cn.Open();
            SqlCommand cmd = cn.CreateCommand();
            cmd.CommandText = "[DailyGlobalSuppressionFtp].[InsertProcessedFtpFile]";
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        public static async Task InsertIntoContactAndDataByEmail(string connectionString)
        {
            try
            {
                await DataLayer.InsertErrorLog(connectionString, 1, "DailyGlobalSuppressionFtpImporter", "DoImport", "[DailyGlobalSuppressionFtp].[InsertIntoContactAndDataByEmail]",
                                "Called from code");

                SqlConnection cn = new SqlConnection(connectionString);
                cn.Open();
                SqlCommand cmd = cn.CreateCommand();
                cmd.CommandText = "[DailyGlobalSuppressionFtp].[InsertIntoContactAndDataByEmail]";
                cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                await DataLayer.InsertErrorLog(connectionString, 100, "DailyGlobalSuppressionFtpImporter", "DoImport", "Failed to run [InsertIntoContactAndDataByEmail]",
                                ex.ToString());
            }
