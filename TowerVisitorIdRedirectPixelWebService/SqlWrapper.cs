...
namespace TowerVisitorIdRedirectPixelWebService
{
    public class SqlWrapper
    {
        public static async Task<string> SqlServerProviderEntry(string conStr, string method, string args, string payload)
        {
            ...
            {
                { "LogPixelFire", "[dbo].[spLogPixelFire]" },
                { "SelectOnMd5", "[dbo].[spSelectOnMd5]" },
                { "AddNewTowerEmail", "[dbo].[spAddNewTowerEmail]" }
            };
        ...
        public static async Task InsertPixelFire(string connectionString, string emailMd5, string label)
        {
            SqlConnection cn = new SqlConnection(connectionString);
            cn.Open();
            SqlCommand cmd = cn.CreateCommand();
            cmd.CommandText = "[dbo].[spLogPixelFire]";
            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.Add(new SqlParameter("EmailMd5", emailMd5));
