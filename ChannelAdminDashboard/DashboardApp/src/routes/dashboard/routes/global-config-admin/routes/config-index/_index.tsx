import { Button, Card, Icon, PageHeader, Table, Dropdown, Menu } from "antd"
...
export function ConfigIndex({ location, navigate, path, uri }: Props): JSX.Element {
            ...
            <Table
              ...
              expandedRowRender={(config) => (
                <MonacoEditor
                  ...
                  language="csharp"
                  theme="vs"
                  value={lookup(config.Id, configDrafts)
                    .chain((draft) => draft.Config)
                    .getOrElse("")}
              ...
              dataSource={configs}
              rowKey="Id"
              onExpand={(didExpand, config) =>
                didExpand ? enableEditingConfig(config) : disableEditingConfig(config)
              }
            />
          </Card>
        </div>
