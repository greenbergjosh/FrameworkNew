...
namespace QuickTester
{
    public static class EdwGrammar3
    {
                ...
                else
                {
                    foreach (var resultEntity in result)
                    {
                        ...
                        if (resultEntity.ValueType == EntityValueType.String)
                            yield return entity.Create(Prefix + (await resultEntity.GetAsS("@")) + Suffix);
                        else
        ...
        public class ConditionInstruction : IEvaluatable
        {
            ...
            public ConditionInstruction(List<(Entity Antecedent, Entity Consequent)> cases)
            {
                Cases = cases;
            }
            ...
            public async IAsyncEnumerable<Entity> Evaluate(Entity entity)
            {
                ...
                {
                    if ((c.Antecedent != null) && (await c.Antecedent.GetB("@")))
                    {
