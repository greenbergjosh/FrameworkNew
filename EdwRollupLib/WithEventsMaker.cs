...
namespace EdwRollupLib
{
    public class WithEventsMaker<T>
    {
        ...
        public Func<T, Task<T>> WithEvents(Func<T, T> dataflowTask, Func<T, Task<string>> stepContextGetter) => async (input) =>
        {
            ...
            await _dropStartEvent(input, dataflowTask.Method.Name, await stepContextGetter?.Invoke(input));
            ...
            {
                await _dropErrorEvent(ex, input, dataflowTask.Method.Name, await stepContextGetter?.Invoke(input), true);
                throw;
            }
            finally
            {
                var end = DateTime.Now;
                await _dropEndEvent(input, end - start, dataflowTask.Method.Name, await stepContextGetter?.Invoke(input));
            }
        };
        ...
        public Func<T, Task> WithEvents(Action<T> dataflowTask, Func<T, Task<string>> stepContextGetter) => async (input) =>
        {
            ...
            {
                await _dropErrorEvent(ex, input, dataflowTask.Method.Name, await stepContextGetter?.Invoke(input), false);
                throw;
            }
            finally
            {
