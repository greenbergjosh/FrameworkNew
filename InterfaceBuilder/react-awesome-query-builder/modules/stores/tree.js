...
import {
  defaultRuleProperties, defaultGroupProperties, defaultFilterProperties, defaultOperator, defaultOperatorOptions, defaultRoot
...
const removeFilter = (state, path, config) => {
  ...
  const parentPath = path.slice(0, -1);
  const isEmptyFilter = !hasChildren(state, parentPath);
  const isEmptyRoot = isEmptyFilter && parentPath.size == 1;
  const canLeaveEmpty = isEmptyFilter && config.settings.canLeaveEmptyFilter && !isEmptyRoot;
  if (isEmptyRoot) {
    state = addItem(state, parentPath, "rule", uuid(), defaultRuleProperties(config), config);
  } else if (!canLeaveEmpty) {
    state = fixEmptyFiltersInTree(state);
  }
  state = fixPathsInTree(state);
  return state;
};
...
export default (config) => {
  ...
  return (state = emptyState, action) => {
    ...
    case constants.ADD_NEW_FILTER:
      return Object.assign({}, state, {...unset}, {tree: addNewFilter(state.tree, action.path, action.properties, action.config)});

    case constants.SET_FILTER_FIELD:
      return Object.assign({}, state, {...unset}, {tree: setFilterField(state.tree, action.path, action.properties, action.config, action.fieldName)});
    ...
    case constants.ADD_FILTER:
      return Object.assign({}, state, {...unset}, {tree: addItem(state.tree, action.path, "filter", action.id, action.properties, action.config)});
