...
namespace DataLayerExtensions
{
    public static class DataTableExtensions
    {
        ...
        {
            NewLighthouseFtpLib.DataLayer.AddDataRow(dt, data);
        }
    }
}
...
namespace NewLighthouseFtpLib
{
    public class DataLayer
    {
        ...
        public static async Task<DateTime> GetMaxEndDayProvidedToThirdParty(string connectionString, string host, int port, string username)
        {
            ...
            SqlDataReader sdr;
            SqlCommand cmd = cn.CreateCommand();
            cmd.CommandText = "NewLighthouseFtp.GetMaxEndDayProvidedToThirdParty";
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        public static async Task<DateTime> GetMaxDateFtpFileProcessed(string connectionString)
        {
            ...
            SqlDataReader sdr;
            SqlCommand cmd = cn.CreateCommand();
            cmd.CommandText = "NewLighthouseFtp.GetMaxDateFtpFileProcessed";
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            DateTime startDayProvidedToThirdParty, DateTime endDayProvidedToThirdParty, string host, int port, string username, int delayInDays)
        {
            ...
            cn.Open();
            SqlCommand cmd = cn.CreateCommand();
            cmd.CommandText = "[NewLighthouseFtp].[InsertThirdPartyExportLog]";
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        public static async Task<Dictionary<byte, DataLayer.FileType>> PopulateFileType(string connectionString)
        {
            Dictionary<byte, DataLayer.FileType> fileTypes = new Dictionary<byte, DataLayer.FileType>();
            SqlConnection cn = new SqlConnection(connectionString);
            ...
            try
            {
                ...
                SqlDataReader sdr;
                SqlCommand cmd = cn.CreateCommand();
                cmd.CommandText = "[NewLighthouseFtp].[FileTypeSelect]";
                cmd.CommandType = CommandType.StoredProcedure;
                sdr = await cmd.ExecuteReaderAsync().ConfigureAwait(continueOnCapturedContext: false);
                while (sdr.Read())
                {
                    fileTypes.Add((byte)sdr["Id"], new DataLayer.FileType()
                    {
                        Id = (byte)sdr["Id"],
            ...
            {
                DataLayer.InsertErrorLogSync(connectionString, 100, "NewLighthouseFtp", "PopulateFileType", "Exception", ex.ToString());
            }
            finally
            {
                if (cn != null) cn.Close();
        ...
        public static async Task<bool> AlreadyDownloaded(string connectionString, string fileName, byte fileTypeId)
        {
            ...
            SqlDataReader sdr;
            SqlCommand cmd = cn.CreateCommand();
            cmd.CommandText = "NewLighthouseFtp.AlreadyDownloaded";
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        public static async Task InsertProcessedFtpFile(string connectionString, byte fileTypeId, string fileName, DateTime fileDate, DateTime startDate, DateTime endDate, long recordsInFile, long recordsAddedFromFile)
        {
            SqlConnection cn = new SqlConnection(connectionString);
            cn.Open();
            SqlCommand cmd = cn.CreateCommand();
            cmd.CommandText = "[NewLighthouseFtp].[InsertProcessedFtpFile]";
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            cmd.Parameters.Add(new SqlParameter("RecordsAddedFromFile", recordsAddedFromFile));
        ...
        public static async Task InsertIntoContactAndDataByEmail(string connectionString)
        {
            try
            {
                await DataLayer.InsertErrorLog(connectionString, 1, "NewLighthouseFtpImporter", "DoImport", "[NewLighthouseFtp].[InsertIntoContactAndDataByEmail]",
                                "Called from code");

                SqlConnection cn = new SqlConnection(connectionString);
                cn.Open();
                SqlCommand cmd = cn.CreateCommand();
                cmd.CommandText = "[NewLighthouseFtp].[InsertIntoContactAndDataByEmail]";
                cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                await DataLayer.InsertErrorLog(connectionString, 100, "NewLighthouseFtpImporter", "DoImport", "Failed to run [InsertIntoContactAndDataByEmail]",
                                ex.ToString());
            }
